...
class ComprehensiveDropdownMenu<T> extends StatefulWidget {
  ...
  ///
  /// The [margin]' top and bottom are ignored, only its left and right
  /// properties are used.
  ///
  /// Defaults to null.
  ...
  ///
  /// ```dart
  /// ComprehensiveDropdownMenu<Text>(
  ///   searchCallback: (List<ComprehensiveDropdownMenuEntry<Text>> entries, String query) {
  ///     if (query.isEmpty) {
  ///       return null;
  ///     }
  ///     final int index = entries.indexWhere((ComprehensiveDropdownMenuEntry<Text> entry) => entry.label == query);
  ///
  ///     return index != -1 ? index : null;
  ///   },
  ///   menuEntries: const <ComprehensiveDropdownMenuEntry<Text>>[],
  /// )
  /// ```
  /// {@end-tool}
  ///
...
class _DropdownMenuState<T> extends State<ComprehensiveDropdownMenu<T>> {
  ...
  setActionButton() {
    ...
    if (actionBtn != null) {
      options.insert(
          widget.actionWidgetPosition == ActionWidgetPosition.top
              ? 0
              : options.length,
          actionBtn!);
    }
  ...
  void didUpdateWidget(ComprehensiveDropdownMenu<T> oldWidget) {
    ...
    if (oldWidget.menuEntries != setActionButton()) {
      currentHighlight = null;
      filteredEntries = setActionButton();
      buttonItemKeys = List<GlobalKey>.generate(
          filteredEntries.length, (int index) => GlobalKey());
  ...
  void refreshLeadingPadding() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      setState(() {
        leadingPadding = getWidth(_cLeadingKey);
      });
    }, debugLabel: 'ComprehensiveDropdownMenu.refreshLeadingPadding');
  }
  ...
  void scrollToHighlight() {
    ...
    }, debugLabel: 'ComprehensiveDropdownMenu.scrollToHighlight');
  }
  ...
  List<ComprehensiveDropdownMenuEntry<T>> filter(
    ...
    if (filteredOption.where((e) => e.value == actionBTN).toList().isEmpty &&
        textEditingController.text.isNotEmpty &&
        actionBtn != null) {
      filteredOption.insert(widget.actionWidgetPosition==ActionWidgetPosition.top?0:filteredOption.length, actionBtn!);
    }
      ...
      final ButtonStyle defaultStyle = switch (textDirection) {
        TextDirection.rtl => MenuItemButton.styleFrom(
            padding: EdgeInsets.only(
                left: kDefaultHorizontalPadding, right: padding)),
        TextDirection.ltr => MenuItemButton.styleFrom(
            padding: EdgeInsets.only(
                left: padding, right: kDefaultHorizontalPadding)),
      };
  ...
  Widget build(BuildContext context) {
    ...
    _initialMenu ??= _buildButtons(setActionButton(), textDirection,
        enableScrollToHighlight: false);
    final DropdownMenuThemeData theme = DropdownMenuTheme.of(context);
    final DropdownMenuThemeData defaults = _DropdownMenuDefaultsM3(context);

    if (_enableFilter) {
      filteredEntries = filter(setActionButton(), _textEditingController);
    }
    ...
    if (widget.enableSearch) {
      if (widget.searchCallback != null) {
        ...
        currentHighlight = search(
            filteredEntries.where((e) => e.value != actionBTN).toList(),
    ...
    final double? anchorWidth = getWidth(_cAnchorKey);
    if (widget.width != null) {
    ...
    Widget menuAnchor = MenuAnchor(
      ...
      builder:
        ...
        final Widget textField = TextField(
            ...
            decoration: InputDecoration(
              ...
              prefixIcon: widget.leadingIcon != null
                  ? Container(key: _cLeadingKey, child: widget.leadingIcon)
                  : null,
        ...
        if (widget.margin != null) {
          ...
          // on its parent width. So we don't need to use `_ComprehensiveDropdownMenuBody` to
          // calculate the children's width.
          return textField;
        }

        return _ComprehensiveDropdownMenuBody(
          width: widget.width,
          children: <Widget>[
            textField,
    ...
    if (widget.margin != null) {
      menuAnchor = Container(
        ...
        padding: widget.margin?.copyWith(top: 0.0, bottom: 0.0),
        child: menuAnchor,
      );
    }
...
class _ComprehensiveDropdownMenuBody extends MultiChildRenderObjectWidget {
  const _ComprehensiveDropdownMenuBody({
    super.children,
    this.width,
  });
  ...
  @override
  _RenderComprehensiveDropdownMenuBody createRenderObject(
      BuildContext context) {
    return _RenderComprehensiveDropdownMenuBody(
      width: width,
    );
  }
  ...
  void updateRenderObject(
      BuildContext context, _RenderComprehensiveDropdownMenuBody renderObject) {
...
class _DropdownMenuDefaultsM3 extends DropdownMenuThemeData {
  ...
  MenuStyle get menuStyle {
    return const MenuStyle(
      minimumSize: MaterialStatePropertyAll<Size>(Size(kMinimumWidth, 0.0)),
...
enum ActionWidgetPosition { top, bottom }
